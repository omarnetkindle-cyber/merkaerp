...
void main() {
  ...
  setUpAll(() async {
    ...
    dbDir = await Directory.systemTemp
        .createTemp('merkaerp_audit_corrections_');
  ...
  group('P1 — Nómina: PayrollParametersService', () {
    ...
    test('seeds 2025 y 2026 existen en la clase y tienen valores razonables',
        () {
    ...
    test('seedOfficialIfAbsent inserta parámetros 2026 y es idempotente',
        () async {
    ...
    test('save persiste parámetros personalizados y los vuelve a leer',
        () async {
      await svc.save(
        companyId: companyId,
        year: 2030,
    ...
    test('liquidarNomina lanza error amigable cuando faltan parámetros',
        () async {
      ...
      expect(
        () => DatabaseHelper.instance
            .liquidarNomina(empleadoId: empId, anio: 2099, mes: 1),
        throwsA(
          predicate(
            (e) =>
  ...
  group('P3 — Navegación: correcciones de módulos', () {
    ...
    test('tax_reports apunta al módulo accounting (no a módulo inexistente)',
        () {
  ...
  group('P9 — Comisiones: corrección doble cálculo', () {
    ...
    test(
      ...
      final commId = await CommissionService.instance.generateCommissionForSale(
        db,
        companyId,
        saleId,
        'FV-$saleId',
  ...
  group('P15 — CreateSaleRequest con campos de vendedor', () {
    test('CreateSaleRequest acepta salespersonId y salespersonName opcionales',
        () {
      final cop = _cop2();
  ...
  group('P10 — Pagos online: contrato de RemotePaymentResult', () {
    test('RemotePaymentResult con providerVerified=true representa pago real',
        () {
  ...
  group('P11 — Integraciones: estrategia por adaptador', () {
    test(
      ...
      expect(true, isTrue, reason: 'DianDirectTransport compila correctamente');
    });
  });
  ...
  group('Precios POS — correcciones previas intactas', () {
    test('10.00 almacenado como 1000 minor units se muestra como "10.00"', () {
      ...
      final displayed = MoneyValue(minorUnits: stored, currency: cop)
          .toMajorUnitsString();
    ...
    test('0.50 almacenado como 50 minor units se muestra como "0.50"', () {
      final cop = _cop2();
      final displayed =
          MoneyValue(minorUnits: 50, currency: cop).toMajorUnitsString();
